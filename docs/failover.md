...
- **Lease (FO-1).** Координатор держит etcd-lease с TTL = `lease_ttl_sec`. Лидер подтверждает лидерство каждым успешным чтением своего appointment'а (обновляет монотонный `last_leader_confirm_mono`). Если за `renew_deadline = lease_ttl − safety_margin` подтверждения нет → **self-fence в полный RO** (sync+async). Сам fence — это `box.cfg{read_only = true}` (блокирует и sync, и async записи). Очередь synchro **освобождается (`box.ctl.demote`) только в контексте `lost_lease`** (etcd жив, наш appointment перехвачен — лидер уже другой). В контексте **`dcs_down`** (etcd недоступен — кворум потерян всем) демоута **нет**: фенсится весь кластер, никто не назначается, и на восстановлении возвращается **тот же** узел — простым `read_only = false`, без re-promote. Демоут здесь обнулил бы limbo лидера, re-promote ушёл бы с `lsn 0`, и отставший follower отверг бы его как `ER_SPLIT_BRAIN` (FO-22b). Настоящий новый лидер, избранный в другой партиции, всё равно побеждает: его PROMOTE с бóльшим term'ом и корректным lsn перекрывает замороженный term на heal.
...
| **DCS-down fence без демоута** (Patroni: RO, keep ownership) | Восстановление после потери etcd-кворума не порождает split-brain re-promote'ом | **FO-22b** self-fence на `dcs_down` делает `read_only=true` без `box.ctl.demote` — владелец очереди сохранён, recovery = чистый `read_only=false`; demote только на `lost_lease` (`fencing.should_release_queue`) |
