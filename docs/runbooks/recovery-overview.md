...
> **Re-bootstrap сохраняет идентичность.** Перед стиранием действие: (1) закрепляет `database.instance_uuid` инстанса в cluster-config (прямой записью в etcd — у переезжаемой ноды репликация часто сломана; правка точечная — `instance_uuid` дописывается в сырой текст конфига без пере-сериализации, поэтому комментарии, порядок ключей и отступы оператора сохраняются, и редактор конфигурации не «схлопывается» после re-bootstrap) и **не** удаляет его строку из `_cluster`, чтобы стёртая нода при рестарте переиспользовала тот же uuid и чисто переехала под своим именем (иначе в именованном кластере Tarantool 3.x rejoin осиротил бы имя, и нода зациклилась бы на `Instance name … is not set in snapshot`, #3740); (2) **глушит** ноду перед wipe — отвязывает репликацию и переводит в read-only, чтобы в окне wipe→exit не дописался WAL под старым uuid (иначе остаётся «лишний» xlog, перематывающий LSN, и пиры отвергают rejoin с `invalid xlog order`). Если закрепить uuid не удалось — re-bootstrap прерывается, не стирая ноду.
