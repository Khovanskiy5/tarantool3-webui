...
├── config_store/            2PC (twophase), schema, diff, history, etcd HTTP client, file-mirror, bootstrap, форматосохраняющие правки (yaml_patch)
...
- **Сохранение форматирования YAML.** Все правки cluster-config идут через `config_store/yaml_patch`, а не через `yaml.decode → yaml.encode` (round-trip стирает комментарии оператора, порядок ключей и отступы, после чего редактор конфигурации показывает «голый» flow-конфиг). Точечные правления одного скаляра (`set_field` / `replace_value`) — rebootstrap identity-pin, election_mode, fix replication URI — редактируют сырой текст напрямую и **fail-safe**: при невозможности заякорить путь возвращают ошибку, а вызывающий прерывается (rebootstrap не стирает ноду). Структурные правки (add/remove инстанса, смена failover-режима) идут через `yaml_patch.render(raw, old, new)`: построчный merge старого текста против старого и нового дерева — меняются только изменившиеся узлы, остальное байт-в-байт. Жёсткая гарантия: результат всегда декодится ровно в `new` (проверяется decode + deep-compare), иначе автоматический откат на `yaml.encode` (валидно, но без комментариев) — то есть никогда не хуже простого re-encode и обычно с полным сохранением комментариев.
