...
-- FO-8: refuse to commit when the etcd control-plane has lost quorum.
-- A write without quorum cannot durably land — failing fast returns a
-- clear error instead of a confusing timeout, and avoids the file-mirror
-- / reload fan-out running against a write that never committed. We only
-- block on a DEFINITE loss (we reached at least one member and a majority
-- is NOT in quorum); a total outage (no member reachable) falls through
-- to the write, whose own error path reports the transport failure.
local function commit_quorum_check(etcd)
    if type(etcd.cluster_health) ~= 'function' then return true end
    local ok_h, health = pcall(function() return etcd:cluster_health() end)
...
-- Write the prepared YAML to etcd (CAS when a revision guard is given,
-- otherwise a plain write). On success drop the prepared entry and
-- return the etcd result; on failure return (nil, err).
local function commit_etcd_write(entry, opts, prepared_id)
...
-- Mirror the YAML to the on-disk file on EVERY peer (atomic rename or
-- in-place truncate-and-write — see file_writer.lua). The file is the
-- recovery source when etcd is unreachable at cold start; keep it in
-- lockstep with the etcd source of truth. Fan-out is best-effort:
-- per-peer outcome is captured in `result.file_mirror` so the resolver
-- can surface partial failures (e.g. one read-only volume on a single
-- peer) without rolling back the etcd commit. etcd is authoritative —
-- the file is the cache.
local function commit_file_mirror(result, payload)
...
-- Opt-in: force `config:reload()` on every peer (and self) after the new
-- YAML has landed in etcd AND on each peer's local file. Without this each
-- peer would only refresh on its own polling tick, which makes a fresh
-- bootstrap UX feel "did anything happen?". Existing call-sites leave the
-- flag off so running clusters pick up changes on their own cadence.
--
-- Best-effort: per-peer outcome is captured in `result.reload_failures`;
-- partial failures do not roll back the etcd commit (etcd is
-- authoritative). Self reload is counted in `reloaded_count` alongside
-- peers.
local function commit_reload_fanout(result, prepared_id, opts)
    if opts.fanout_reload ~= true then return end
...
-- Record the snapshot in our own /history/ timeline, keyed by etcd's
-- commit revision so the timeline maps 1:1 to what `:get('config')` would
-- return at that point. Soft failure: a history write blowing up must NOT
-- roll back the commit itself — "committed" means "the cluster will pick
-- it up", not "the timeline panel will render it".
local function commit_history(result, entry, opts, payload)
    if result.revision == nil then return end
    pcall(function()
...
-- Fire-and-forget notification that a config commit landed.
local function commit_notify(entry, result)
    pcall(function()
...
--
-- Linear pipeline: lookup → quorum guard → etcd write → file mirror →
-- optional reload fan-out → history snapshot → notification. Only the
-- lookup / quorum / write phases can fail the commit; everything after
-- the etcd write is best-effort (etcd is authoritative).
function M.commit(prepared_id, opts)
    ...
    local qok, qerr = commit_quorum_check(opts.etcd)
    if not qok then return nil, qerr end

    local result, err = commit_etcd_write(entry, opts, prepared_id)
    if result == nil then return nil, err end

    commit_file_mirror(result, entry.yaml)
    commit_reload_fanout(result, prepared_id, opts)
    commit_history(result, entry, opts, entry.yaml)
    commit_notify(entry, result)
    return result
end
