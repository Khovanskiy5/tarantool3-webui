...
-- Source maps are emitted by Vite as `sourcemap: 'hidden'` — present on
-- disk but not referenced from the bundle. They are huge (Monaco alone
-- ships 12+ MiB of maps) and have no purpose at runtime; error trackers
-- consume the .map files directly from `dist/` before deploy. Skipping
-- them keeps the embedded bundle under control without affecting served
-- assets.
local function is_sourcemap(rel)
    return string.sub(rel, -4) == '.map'
end

-- Walk the source tree and group each canonical asset with its
-- pre-compressed companions. Each canonical asset has up to three
-- variants: raw (the asset itself), br (asset.br, brotli) and gz
-- (asset.gz, gzip). The returned table is keyed by the canonical
-- relative path (forward slashes) and stores absolute paths per variant.
local function collect_entries(source_dir)
    local entries = {}
    for _, item in ipairs(walk(source_dir)) do
        local rel = string.gsub(item.rel, '\\', '/')
        if not is_sourcemap(rel) then
            local key, kind
...
local function bundle_preamble(source_dir)
    return {
...
local function add_entry(lines, route, mime, etag, raw_b, br_b, gz_b)
...
-- Read the raw asset plus any present compressed companions. A read
-- failure on any variant is fatal (os.exit 2) — the bundle must be
-- complete or not produced at all.
local function read_asset_variants(paths)
    local raw, raw_err = read_file(paths.raw)
    if raw == nil then
        io.stderr:write(string.format('embed-assets: ERROR %s\n', raw_err))
        os.exit(2)
    end
    local br = nil
    if paths.br then
...
-- Append one asset's table entry to `lines` and fold its sizes into the
-- `stats` accumulator (count / totals / report rows).
local function emit_asset(lines, key, raw, br, gz, stats)
    local route = '/' .. key
    local mime = mime_for(key)
    local etag = '"' .. digest.sha1_hex(raw) .. '"'

    add_entry(lines, route, mime, etag, raw, br, gz)
    stats.count = stats.count + 1
    stats.total_raw = stats.total_raw + #raw
    if br then stats.total_br = stats.total_br + #br end
    if gz then stats.total_gz = stats.total_gz + #gz end

    table.insert(stats.report_rows, {
        route = route,
        mime = mime,
        raw = #raw,
        br = br and #br or 0,
...
-- Render every canonical asset (in sorted order) into `lines`, updating
-- `stats`. Orphan .br/.gz without a sibling raw file are skipped with a
-- warning — Vite always emits the raw alongside compressed variants, so
-- an orphan is usually a build-cleanup glitch worth surfacing.
local function render_assets(ordered_keys, entries, lines, stats)
    for _, key in ipairs(ordered_keys) do
        local paths = entries[key]
        if paths.raw == nil then
            stats.skipped_no_raw = stats.skipped_no_raw + 1
            io.stderr:write(string.format(
                'embed-assets: WARN no raw for %s (br=%s gz=%s); skipping\n',
                key, tostring(paths.br ~= nil), tostring(paths.gz ~= nil)
            ))
        else
            local raw, br, gz = read_asset_variants(paths)
            emit_asset(lines, key, raw, br, gz, stats)
        end
    end
end
...
local function write_output(lines, output_file)
    local out_dir = fio.dirname(output_file)
    if not fio.path.is_dir(out_dir) then
        fio.mktree(out_dir)
    end

    local out_f = fio.open(output_file,
...
-- Pretty per-asset report, stable and sorted by route, followed by the
-- totals row and the on-disk output size.
local function print_report(stats, source_dir, output_file)
    io.stdout:write(string.format(
        'embed-assets: bundled %d asset(s) from %s\n', stats.count, source_dir
    ))
    ...
    for _, r in ipairs(stats.report_rows) do
    ...
    io.stdout:write(string.format(
        '%-60s %-44s %10s %10s %10s\n',
        'TOTAL', '', human(stats.total_raw), human(stats.total_br),
        human(stats.total_gz)
    ))
    ...
    if stats.skipped_no_raw > 0 then
        io.stdout:write(string.format(
...
local entries = collect_entries(SOURCE_DIR)
local ordered_keys = sorted_keys(entries)

local lines = bundle_preamble(SOURCE_DIR)
local stats = {
    count = 0,
    total_raw = 0,
    total_br = 0,
...
render_assets(ordered_keys, entries, lines, stats)

table.insert(lines, '}')
table.insert(lines, '')

write_output(lines, OUTPUT_FILE)
print_report(stats, SOURCE_DIR, OUTPUT_FILE)
